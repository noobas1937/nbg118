...
def main():
    ...
    userclasses_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'IF', 'Classes'))
    # save config to file
    ...
    config.set('DEFAULT', 'userdir', userclasses_root)
    ...
    try:
        ...
        output_dir = userclasses_root

        cmd_args = {
                    'userclasses.ini' : ('userclasses', 'lua_userclasses_auto'), \
                    }
        target = 'lua'
