...
void main()
{
	...
	v_reflectionPower = clamp((1.0 - length(vec2(a_position.x * 0.7 + (a_texCoord.x - 0.5) * 1.5, a_position.y * ystretch) - vec2(0.0, ystretch))) * 3.0, 1.3, 1.6);
	
    float uvFactor = 0.6;
	float x = v_texcoord0.x;
	float y = v_texcoord0.y * uvFactor;
...
#endif // SIMPLE
/*
	float blendForce = (a_position.y * 0.5 + 0.1) * 0.3;
