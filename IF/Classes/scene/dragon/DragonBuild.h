...
#ifndef __IF__DragonBuild__
#define __IF__DragonBuild__
...
class DragonBuildState;

class DragonBuild: public CCAniNode
,public CCBMemberVariableAssigner
,public CCBSelectorResolver
{
public:
    DragonBuild():m_key(0),m_type(0),isShowUping(false),mainWidth(0),mainHeight(0),m_buildingKey(0),m_info(NULL),isChange(true)
    ...
    ,m_nameLayer(NULL),m_signLayer(NULL),m_popLayer(NULL),m_batchNode(NULL),m_blentBatchNode(NULL),m_tipNode(NULL){};
    ~DragonBuild(){};
    bool initDragonBuild(int itemId, CCLabelBatchNode *nameLayer);
    static DragonBuild* create(int itemId, CCLabelBatchNode *nameLayer);
    
    CCNode* getGuideNode(string _key);
    void showTmpBuild(int itemId);
    void hideTmpBuild();
    static DragonBuild* createTmpBuild(int itemId, int x, int y, CCSpriteBatchNode* batchNode, int zOrder, CCSpriteBatchNode* blentbatch=NULL);
    ...
    void canShowState();
    void setNamePos(int x, int y, CCLayer* sginLayer, CCLayer* popLayer, CCSpriteBatchNode* batchNode, int zOrder, CCSpriteBatchNode* blentbatch=NULL);
    void addTips();
    void removeTips();
...
protected:
    ...
    CCSafeObject<DragonBuildState> m_buildState;
    ...
    void addDragonBuildState();
    void removeDragonBuildState();
...
class DragonBuildState: public CCAniNode
,public CCBMemberVariableAssigner
,public CCBSelectorResolver
 
{
public:
    DragonBuildState():CanDel(true),m_info(NULL),m_buildingKey(0),isChange(true),m_qType(0),m_qid(QID_MAX),m_sumTime(1),m_curTime(0){};
    ~DragonBuildState(){};
    bool init(int itemId);
    static DragonBuildState* create(int itemId);
...
#endif /* defined(__IF__DragonBuild__) */
