...
public class CombineBitmapManager
{
	...
	public Bitmap getCombinedBitmap(ArrayList<Bitmap> bitmapArray)
	{
		...
		{
			bitmaps[i] = ThumbnailUtils.extractThumbnail(bitmaps[i], (int) entityList
					.get(0).width, (int) entityList.get(0).width);
		}
...
//		count = 8;
		...
		for (int i = 0; i < count; i++)
		{
			mBitmaps[i] = ThumbnailUtils.extractThumbnail(BitmapUtil.getScaleBitmap(
					context.getResources(), resIDs[MathUtil.random(0, resIDs.length - 1)]), (int) entityList
					.get(0).width, (int) entityList.get(0).width);
		}
		Bitmap combineBitmap = BitmapUtil.getCombineBitmaps(entityList, mBitmaps);
//		try {
//			BitmapUtil.saveMyBitmap(context, combineBitmap, "", "thumb" + count);
//		} catch (IOException e) {
//			System.out.println("save png accur error");
//			e.printStackTrace();
//		}
		return combineBitmap;
	}
	...
	private List<MyBitmapEntity> getBitmapEntitys(int count, double totalWidth, double gap) {
		...
		if(count <= 0 || totalWidth <= 0) return mList;
	...
	private static double getXPos(int index, int count, int rowCount, int columnCount, double gap, double width, double totalWidth)
	{
		...
		if(rowColumnCnt > columnCount) rowColumnCnt = columnCount;
