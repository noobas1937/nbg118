...
//		e.printStackTrace();
		...
		if (stacks != null)
		{
			...
			for (int i = 0; i < stacks.length; i++)
			{
				...
				LogUtil.printVariables(Log.WARN, LogUtil.TAG_ALL, "    " + stack);
				result += "\n    " + stack;
				if (StringUtils.isEmpty(function) && StringUtils.isNotEmpty(stack) && stack.contains("com.elex.chatservice."))
				{
					function = stack.replace("com.elex.chatservice.", "");
				}
			}
	...
	public static void trackPageView(String page, String action)
	{
		if (tracker != null)
		{
			tracker.trackMessage("ChatPageView", "page", page, "action", action);
		}
	}
	...
	private static void printVariablesWithFuctionName(int priority, String tag, boolean showFunctionName, boolean showIndent, Object... args)
	{
		...
		if (showFunctionName)
		{
			output = getFunctionName(false) + ": \n" + getVariablesOutput(showIndent, args);
		}
		else
		{
