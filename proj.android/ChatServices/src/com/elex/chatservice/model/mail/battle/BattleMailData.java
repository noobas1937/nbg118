	...
	public void parseContents() {
		...
		if(!getContents().equals(""))
		{
			try {
				...
				if(detail.getAtkUser()!=null && detail.getAtkUser().getName()!=null && UserManager.getInstance().getCurrentUser()!=null)
				{
					if(UserManager.getInstance().getCurrentUser().userName.equals(detail.getAtkUser().getName()))
					{
						contentText=LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_ATTACK, LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_MYARMY), defUserName);
					}
					else
					{
						contentText=LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_ATTACK, detail.getAtkUser().getName(), LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_MYCASTLE));
					}
				}
				...
				if(isAtt){
		            contentText = LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_ATTACK,LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_MYARMY),defName);
		        }else{
		            if(detail.getBattleType()!=3){
		            	contentText = LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_ATTACK,attName,LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_MYARMY));
		            }else{
		            	contentText = LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_ATTACK,attName,LanguageManager.getLangByKey(LanguageKeys.MAIL_TITLE_MYCASTLE));
		            }
		        }
